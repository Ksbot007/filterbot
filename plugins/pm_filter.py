...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [
            InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url=':https://t.me/ksbots/6')
        ]
    )
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('THIS MOVIE IS NOT ADDED IN MY DATABASE TAG ADMIN 💌')
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"**Here is What I Found In My Database For Your Query {search}**"
